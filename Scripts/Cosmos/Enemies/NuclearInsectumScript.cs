...
public class NuclearInsectumScript : MonoBehaviour
{
    ...
    public AudioClip acidicSporeSound;
    ...
    public IEnumerator damageShip(int dmg)
    {
        ...
        healthBarRed.fillAmount = health / 70f;
        ...
        if (health <= 0)
        {
            ...
            PlayerPrefs.SetString("Planet Completed", "Frater Minor Completed");
    ...
    {
        isTorpedoing = true;
        AudioSource.PlayClipAtPoint(acidicSporeSound, camera.position, PlayerPrefs.GetFloat("SFX Volume"));
        if (whichBarrel % 2 == 0)
        {
            Instantiate(acidicSpore, blasterThree.position, transform.rotation);
        }
        else
        {
            Instantiate(acidicSpore, blasterFour.position, transform.rotation);
        }
        whichBarrel++;
