...
public class UltusDreadnoughtScript : MonoBehaviour
{
    ...
    public IEnumerator damageShip(int dmg)
    {
        ...
        healthBarRed.fillAmount = health / 50f;
        ...
        if (health <= 0)
        {
            ...
            PlayerPrefs.SetString("Planet Completed", "Captiosus Completed");
    ...
    IEnumerator shoot(float waitTime)
    {
        ...
        Quaternion targetRotation = Quaternion.Euler(new Vector3(0, 0, Random.Range(-15, 15)));
        if (otherRandInt == 0)
        {
            Instantiate(laser, blaster.position, transform.rotation * targetRotation);
            Instantiate(muzzleBlast, blaster.position, transform.rotation * targetRotation);
        }
        else if (otherRandInt == 1)
        {
            Instantiate(laser, blasterTwo.position, transform.rotation * targetRotation);
            Instantiate(muzzleBlast, blasterTwo.position, transform.rotation * targetRotation);
        }
        else
        {
            {
                Instantiate(laser, blasterThree.position, transform.rotation * targetRotation);
                Instantiate(muzzleBlast, blasterThree.position, transform.rotation * targetRotation);
            }
        }
